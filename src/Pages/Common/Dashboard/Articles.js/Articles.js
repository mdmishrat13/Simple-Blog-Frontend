  ...
  return (
    <div className="z-1 p-4">
        ...
        <div className="flex justify-between">
          ...
          <button>
            <Link
              ...
              className="px-4 py-2 bg-gray-600 text-white cursor-pointer hover:bg-gray-700 flex item-center"
            >
      ...
      <table className="text-sm w-full text-left text-gray-500 p-2">
        ...
        <tbody>
          {myPosts.map((post) => (
            <Article deletePost={deletePost} post={post} key={post._id}></Article>
          ))}
        </tbody>
      </table>
      {!myPosts.length && (
        <h1 className="text-center p-4 text-xl bg-white"> You have no post</h1>
      )}
    </div>
  );
};
